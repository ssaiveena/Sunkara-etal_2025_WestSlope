...
#size= comm.Get_size()
#start_realization = np.arange(0,100,1) #changed 1000 to 100 to 10 in the middle
#assigned_scenario = start_realization[rank%100]
#for i in range(20):
#scenario = 'S_{}'.format(assigned_scenario)
    #os.chdir(projectdirectory + 'Experiment_files/' + 'S_{}'.format(start_realization[rank]) + '/')
#if not os.path.exists('Experiment_files/' +'S_{}'.format(start_realization[rank]) + '/parquet_files'):
 #   os.makedirs('Experiment_files/' +'S_{}'.format(start_realization[rank]) + '/parquet_files')
#converter = XddConverter(
    # path to a directory where output .parquet files should be written
#output_path= 'Experiment_files/' + scenario + '/parquet_files/',
    # whether to abort if .parquet files already exist at the output_path
#allow_overwrite=True,
    # path, glob, or a list of paths/globs to the .xdd files you want to convert
#xdd_files='Experiment_files/' + scenario + "/cm*.xdd",
    # if the output .parquet files should only contain a subset of structure ids, list them here; None for all
#id_subset=None,
    # how many .xdd files to convert in paralllel; optimally you will want 2-4 CPUs per parallel process
#parallel_jobs=1,
...
'''
# loop through realizations and convert xdds to parquet
for j in range(int(NUM_RELS)):
    scenario = 'S{}_1'.format(rank +  int(N_PROC) * j)
    print(scenario)
    #if not os.path.exists(projectdirectory + '/parquet/'+scenario + '/'):
    #    os.makedirs(projectdirectory + '/parquet/'+scenario + '/')

    converter = XddConverter(
        # path to a directory where output .parquet files should be written
        output_path= scenario + '/parquet/',
        # whether to abort if .parquet files already exist at the output_path
        allow_overwrite=True,
        # path, glob, or a list of paths/globs to the .xdd files you want to convert
        xdd_files=scenario + "/*.xdd",
        # if the output .parquet files should only contain a subset of structure ids, list them here; None for all
        id_subset=None,
        # how many .xdd files to convert in paralllel; optimally you will want 2-4 CPUs per parallel process
        parallel_jobs=1,
    ...
    # delete the original xdd files to save space
    #os.remove(scenario + '/' + BASIN_NAME +'2015B_'+ scenario + '.xdd')

    # move the parquet file to the general parquet folder
    # os.replace(scenario + '/parquet/'+ BASIN_NAME +'2015B_'+ scenario + '.parquet', 'parquet/' + BASIN_NAME + '2015B_' + scenario + '.parquet')
    '''
#comm.Barrier()
